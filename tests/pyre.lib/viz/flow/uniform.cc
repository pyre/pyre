...
// the factory
using uniform_t = pyre::viz::factories::filters::uniform_t<signal_t, bin_t>;
...
main(int argc, char * argv[])
{
    ...
    // make the factory
    auto uniform = uniform_t::create("uniform", 10);
    // wire it
    ...
    // pick another number of bins and modify the factory
    uniform->bins(100);
    // go through the tile contents
    for (auto v : bin->read()) {
