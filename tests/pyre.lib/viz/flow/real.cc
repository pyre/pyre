...
// factories
using real_t = pyre::viz::factories::selectors::real_t;
...
main(int argc, char * argv[])
{
    ...
    // the real
    auto real = channel_t::create("real", shape, 0.0);
    ...
    // make the selector
    auto selector = real_t::create("real");
    // wire it
    ...
    // open a file
    auto stream = std::ofstream("real.bmp", std::ios::out | std::ios::binary);
    // if successful
    if (stream.is_open()) {
        // write the bytes
