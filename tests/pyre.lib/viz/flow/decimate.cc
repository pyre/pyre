...
main(int argc, char * argv[])
{
    ...
    // make the input data large enough
    auto signal = signal_t::create("signal", factor * shape, 0.0);
    // its data grid
    ...
    auto decimated = signal_t::create("decimated", shape, 0.0);
    ...
    // make the decimator
    auto filter = decimated_t::create("decimator", level);
    // wire it
    filter->signal(signal);
    filter->decimated(decimated);

    // make the colorspace
    auto color = color_t::create("complex", 360, 1.0, .25, 1.0);
    // wire it
    color->signal(decimated);
    color->red(red);
    color->green(green);
    color->blue(blue);
    ...
    // open a file
    auto stream = std::ofstream("pyre_viz_flow_decimate.bmp", std::ios::out | std::ios::binary);
    // if successful
    if (stream.is_open()) {
        // write the bytes
