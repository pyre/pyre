...
"""
The constrained mpi edge overrides the mpi preference order
"""
...
def test():
    """
    Demand the mpich flavor of hdf5 and verify mpi follows it
    """
    # the engine to fake
    ...
    index = Index()
    # wire it
    index._engines = (engine(name="dpkg.fake.hdf5.alignment"),)
    # demand the mpich flavor of hdf5, saying nothing about mpi itself
    report = index.resolve(requested=["hdf5[mpich]"])
    # the demand picked the mpich build
    assert report.selections["hdf5"].flavor == "hdf5-mpich"
    # and its induced edge overrode mpi's own preference for openmpi
