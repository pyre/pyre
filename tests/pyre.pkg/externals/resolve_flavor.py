...
"""
A flavor selector overrides the recipe preference order
"""
...
def test():
    """
    Demand the non-preferred flavor and verify it wins the selection
    """
    # the engine to fake
    ...
    index = Index()
    # wire it
    index._engines = (engine(name="dpkg.fake.flavor"),)
    # resolve a request that names the non-preferred flavor
    report = index.resolve(requested=["mpi[mpich]"])
    # the selection must honor the demand, not the preference order
