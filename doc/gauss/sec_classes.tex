...
\lstlineref{mt:unpack}.  The \function{zip} on \lstlineref{mt:zip} returns a list of tuples,
...
\lstlineref{mt:list} in order to extract the arguments to \function{uniform}.
...
representation as before. The new implementation of \class{MersenneTwister} is
%
\python{
  firstline=9,lastline=32,
...
language \identifier{Icon} \cite{icon} that is now available in python.\footnote{Generators
  were first seen in version 2.2 and turned into a core part of the language in version 2.3}
...
description, let's revisit \class{MersenneTwister}. The improvements we will discuss in this
...
implementation of \class{MersenneTwister} is shown in \lstref{classes:mt:generators}.
%
%\begin{python}[%
  %label={lst:classes:mt:generators},
...
value on \lstlineref{mt:generators:return}. Indeed, we unpack the arguments to \method{point}
...
follows the \keyword{yield} statement on \lstlineref{mt:generators:yield} that suspended it. In
