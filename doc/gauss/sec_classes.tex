...
will fit this bill very nicely, and its specialization \class{WichmannHill} will create point
clouds using the Wichmann-Hill random number generator we used in \secref{simple:python}. 
...
implementation; instead \method{point} raises an exception to remind developers of derived
...
The class \class{PointCloud} and its derivatives should be generally useful to us and are good
...
would suggest an abstract base class \class{Shape} with a single method:
%
\python{
  firstline=9,lastline=21,
  label={lst:classes:shape},
  caption={\srcfile{classes-simple/Shape.py}: The base class for the representation of the
    region of integration},
}{listings/classes-simple/Shape.py}
%
from which we will derive a class \class{Disk} that will represent the interior of a circle.
...
\python{
  ...
  caption={\srcfile{classes-simple/Disk.py}: Representing our region of integration},
...
understand, and they expose users of the class \class{Disk} to the risk that the default
...
%
\python{
  firstline=9,lastline=32,
  label=lst:classes:driver,
  caption={\srcfile{classes-simple/gauss.py}: The driver for the objected oriented solution},
...
\lstref{classes:wh} and \lstref{classes:disk}, you will find quite a bit of code associated
...
Turning to the implementation, \class{PointCloud} would look like
%
\python{
  firstline=9,lastline=24,
  label={lst:classes:pointcloud:containers},
  caption={\srcfile{classes-containers/PointCloud.py}: An improved class for representing the
    evaluation points},
...
\python{
  ...
  caption={\srcfile{classes-containers/WichmannHill.py}: Filling a container with random points},
...
\python{
  ...
  caption={\srcfile{classes-containers/Shape.py}: The base class \class{Shape} requires little
    change},
}{listings/classes-containers/Shape.py}
%
The implementation of \class{Disk} is a bit more complicated than \lstref{classes:shape}.
%
\python{
  firstline=9,lastline=37,
  label={lst:classes:disk:containers},
  caption={\srcfile{classes-containers/Disk.py}: Implementing \class{Disk} as a filter},
...
integration driver, is the absence of counters. Instead, we have used \function{sum} that
adds up the elements of its input list and converts the filtered points from \class{Disk} into
...
the version of \class{PointCloud} shown in \lstref{classes:pointcloud:containers}. The new
...
%
%\begin{python}[%
  %label={lst:classes:disk:generators},
...
result in \identifier{mask} as we did in \lstref{classes:disk:containers}, we either
...
\lstref{classes:disk:generators} does not know or care whether the point container it is
...
%\begin{python}[%
  ...
  %label={lst:classes:pointcloud-final}
%]{}
%class Random(object):
%
    %def seed(self, n):
