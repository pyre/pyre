...
language \identifier{Icon} \cite{icon}.\footnote{Generators were first seen in version 2.2 and
became a core part of the language in version 2.3} Hopefully, we will be able to avoid
any confusion between random number generators and the programming construct.
...
constructs in python have strange physiological effects\cite{guido}.
...
All this comes together in the driver show in \lstref{generators:driver}.  See if you
can follow how looping over the virtual contents is driving the generators to produce the
associated values without paying the computational expense to set up the function or manage a
container for the intermediate values.
