...
of \keyword{True} or \keyword{False} values indicating whether the corresponding point was
interior or not. Keep in mind that it is not acceptable to have \method{interior} just count
the number of interior points for us, for a couple of reasons. First, this capability pollutes
the design of \class{Shape} subclasses and specializes them too much for reuse. Second, we will
eventually need to know which points are interior to the \class{Shape} so we can evaluate our
integrand $f$ there.
...
%
The implementation of \class{Disk}, shown in \lstref{classes:disk:containers}, is a bit more
complicated than \lstref{classes:disk}. In \method{interior}, we initialize the container
\identifier{mask} and populate it with \keyword{True} or \keyword{False} to indicate whether
each point lies on the interior of the disk.
%
\python{
...
%
Again, we have to pay for the cost of managing \identifier{mask}, so we must be very careful
when we manipulate its entries.
...
A striking difference between \lstref{classes:driver} and the other implementations of the
