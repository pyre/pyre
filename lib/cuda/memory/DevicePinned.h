...
// code guard
#if !defined(pyre_cuda_memory_DevicePinned_h)
#define pyre_cuda_memory_DevicePinned_h

// a block of cells on pinned memory
template <class T, bool isConst>
class pyre::cuda::memory::DevicePinned {
    // types
public:
    // my cell
...
public:
    ...
    inline DevicePinned(cell_count_type);
    // i can make one from a block and a count
    inline DevicePinned(handle_type, cell_count_type);
...
public:
    // destructor
    ~DevicePinned() = default;
    // constructors
    DevicePinned(const DevicePinned &) = default;
    DevicePinned(DevicePinned &&) = default;
    DevicePinned & operator=(const DevicePinned &) = default;
    DevicePinned & operator=(DevicePinned &&) = default;
};

// get the inline definitions
#define pyre_cuda_memory_DevicePinned_icc
#include "DevicePinned.icc"
#undef pyre_cuda_memory_DevicePinned_icc
