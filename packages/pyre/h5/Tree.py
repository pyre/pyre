...
class Tree:
    ...
    def explore(self, location, margin="", graphic=""):
        ...
        for child in children[:-1]:
            ...
            yield from self.explore(location=child, margin=bodyMargin, graphic=bodyGraphic)

        # the last child gets decorated slightly differently
        lastMargin = margin + '   '
        lastGraphic = margin + '`- '
        # repeat for the last child
        yield from self.explore(location=children[-1], margin=lastMargin, graphic=lastGraphic)
