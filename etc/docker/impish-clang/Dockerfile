## -*- docker-image-name: "pyre:impish-clang" -*-
#
...
COPY etc/docker/impish-gcc/inputrc .inputrc
# the startup file
COPY etc/docker/impish-gcc/bashrc bashrc.in
# expand
RUN sed \
        -e "s:@SRCDIR@:${srcdir}:g" \
        bashrc.in > .bashrc
...
COPY etc/docker/impish-gcc/mpi.pfg mpi.pfg
# the {mm} configuration file
COPY etc/docker/impish-clang/mm.pfg mm.pfg.in
# expand
RUN sed \
        -e "s:@PREFIX@:${prefix}:g" \
        mm.pfg.in > mm.pfg
...
COPY etc/docker/impish-clang/config.mm config.mm.in
# expand
RUN sed \
